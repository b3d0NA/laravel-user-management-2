...

{
    ...
    public function company_id()
    {
        ...
        switch ($determinator) {
            case 0:
                $company_idName = static::randomElement(static::$product) . ' ' . static::randomElement(static::$type);

                break;

            case 1:
                $company_idName = static::randomElement(static::$product) . strtolower(static::randomElement(static::$type));

                break;

            case 2:
                $company_idName = static::randomElement(static::$store) . ' ' . $this->generator->lastName();

                break;
        }

        if (Miscellaneous::boolean()) {
            return $company_idName . ' ' . static::randomElement(static::$company_idSuffix);
        }

        return $company_idName;
    }
